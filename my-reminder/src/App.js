...
function App() {
  ...
  const deleteTask = async (id) => {
    const newTasks = await tasks.filter((task) => 
      task.id !== id
    )
    setTasks(newTasks);
  ...
  return (
    <div className="app">
      ...
      {
       tasks.length > 0 ? <Tasks onDouble={onDoubleClick} onDelete={deleteTask} tasks={tasks} /> : <h4 style={{ color: "red" }}>No task to show</h4>
      }
    </div>
  );
}
